...
Digital 52, INPUT_PULLUP

Start button

Digital 35, INPUT_PULLUP; pressed = LOW

Stop button

Digital 31, INPUT_PULLUP; pressed = LOW
...
Motor shield M2 (cfpump)

Pump 2

Motor shield M4 (pump)

Additional motor

Motor shield M1 (motor)
...
Save the Arduino source as Water-Filtration/Water-Filtration.ino so the sketch filename matches its containing folder.

Install the Arduino IDE and the AFMotor library compatible with the team's motor shield. The Servo library is also required.

Select Arduino Mega or Mega 2560 and the appropriate port, then compile the sketch. Confirm the shield and board combination works before connecting pumps or a motor.

Set the Serial Monitor to 9600 baud. Press the start button to begin; use the stop button to halt after the current blocking operation finishes.
